...
class Attention(nn.Module):
    def __init__(self, args, in_feature, ratio=4):
        ...
        self.FA1 = Block(dim=196, num_heads=14, mlp_ratio=4.0, qkv_bias=True, qk_scale=None, drop_ratio=0,
                         attn_drop_ratio=0, drop_path_ratio=0, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                         act_layer=nn.GELU, num_patches=768)
    ...
    def forward(self, x):
        ...
        # feature_map = feature_map.view(bs, c, -1)
        # feature_map = feature_map.permute(0, 2, 1)
        # feature_map = self.SA1(feature_map)
        # feature_map = feature_map.permute(0, 2, 1)
        # feature_map = self.SA2(feature_map)
        # feature_map = feature_map.view(bs, c, 14, 14)

        # 加权操作，SA，FA
        bs, c, _, _ = feature_map.shape
        feature_map_a1 = feature_map.view(bs, c, -1)
...
class Attention1(nn.Module):
    ...
    def forward(self, x):
        ...
        feature_map = feature_map.view(bs, c, -1)
        feature_map = feature_map.permute(0, 2, 1)
        feature_map, _ = self.SA1(feature_map)
        feature_map = feature_map.permute(0, 2, 1)
        feature_map, _ = self.FA1(feature_map)
        feature_map = feature_map.view(bs, c, 14, 14)
        ...
        # 串联并联一起
        # bs, c, _, _ = feature_map.shape
        # feature_map_c1 = feature_map.view(bs, c, -1)
        # feature_map_c1 = feature_map_c1.permute(0, 2, 1)
        # feature_map_c1, _ = self.SA1(feature_map_c1)
        # feature_map_c1 = feature_map_c1.permute(0, 2, 1)
        # feature_map_c1, _ = self.FA1(feature_map_c1)
        # feature_map_c1 = feature_map_c1.view(bs, c, 14, 14)
        #
        # bs, c, _, _ = feature_map.shape
        # feature_map_a1 = feature_map.view(bs, c, -1)
        # feature_map_a1 = feature_map_a1.permute(0, 2, 1)
        ...
        # feature_map_a2 = feature_map_a2.view(bs, c, 14, 14)
        #
        # feature_map_c2 = feature_map_a1 + feature_map_a2
        #
        # feature_map = (feature_map_c1 + feature_map_c2) / 2
...
class SAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale  # attn = [bs, head, token_num, token_len/head]

        # 在这里开始丢头
        # drop = []
        # for t in range(4):
        #     i = randint(0, self.num_heads - 1)
        #     drop.append(i)
        # for i in drop:
        #     attn[:, i, :, :] = 0

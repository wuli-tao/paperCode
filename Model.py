...
class AConv(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        ...
        # 动态卷积
        self.conv1 = DynamicConv(args, 64, 4, True)
        self.conv2 = DynamicConv(args, 128, 4, True)
        self.conv3 = DynamicConv(args, 256, 4, True)
        self.conv4 = DynamicConv(args, 512, 4, True)
...
class DHSA(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        ...
        # 动态卷积
        self.conv1 = DynamicConv_DHSA(args, 64, 4, False, False)
        self.conv2 = DynamicConv_DHSA(args, 128, 4, False, False)
        self.conv3 = DynamicConv_DHSA(args, 256, 4, False, False)
        self.conv4 = DynamicConv_DHSA(args, 512, 4, False, False)
    ...
    def forward(self, x):
        ...
        featuremap = (featureMap1 + featureMap2 + featureMap3 + featureMap4) / 4
        score = score1 + score2 + score3 + score4
...
class AConv_DHSA(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        super(AConv_DHSA, self).__init__()
        self.drop_rate = drop_rate
        ...
        # 动态卷积
        self.conv1 = DynamicConv(args, 64, 4, False, False)
        self.conv2 = DynamicConv(args, 128, 4, False, False)
        self.conv3 = DynamicConv(args, 256, 4, False, False)
        self.conv4 = DynamicConv(args, 512, 4, False, False)
...
class AConv_DHSA_global(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        super(AConv_DHSA_global, self).__init__()
        self.drop_rate = drop_rate
        ...
        # 动态卷积
        self.conv1 = DynamicConv(args, 64, 4, False, True)
        self.conv2 = DynamicConv(args, 128, 4, False, True)
        self.conv3 = DynamicConv(args, 256, 4, False, True)
        self.conv4 = DynamicConv(args, 512, 4, False, True)
...
class AConv_DHSA_global_x(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        ...
        self.bn = nn.BatchNorm2d(512)
        self.relu = nn.ReLU(inplace=True)
        self.alpha = nn.Parameter(torch.ones(1) * 10)
    ...
    def forward(self, x):
        ...
        featuremap = featureMap1 + featureMap2 + featureMap3 + featureMap4
        featuremap = torch.cat((featuremap, self.upsample(x)), dim=1)
        # featuremap = featuremap + self.upsample(x)

        feature = self.avgpool(featuremap)
        feature = feature.view(feature.size(0), -1)
        feature = feature / torch.norm(feature, p=2, dim=1, keepdim=True) * self.alpha
        out = self.fc(feature)
...
class test(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        ...
        # 动态卷积
        self.conv1 = DynamicConv1(args, 64, 4, False, False)
        self.conv2 = DynamicConv1(args, 128, 4, False, False)
        self.conv3 = DynamicConv1(args, 256, 4, False, False)
        self.conv4 = DynamicConv1(args, 512, 4, False, False)

        self.SA1 = Block(dim=512, num_heads=16, mlp_ratio=4.0, qkv_bias=True, qk_scale=None, drop_ratio=0,
                         attn_drop_ratio=0, drop_path_ratio=0, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                         act_layer=nn.GELU, num_patches=196)
    ...
    def forward(self, x):
        ...
        temp1 = torch.cat((featuremap1, featuremap2), 2)
        temp2 = torch.cat((featuremap3, featuremap4), 2)
        featuremap = torch.cat((temp1, temp2), 3)

        bs, c, _, _ = featuremap.shape
        feature_map_1 = featuremap.view(bs, c, -1)
        ...
        featuremap = feature_map_1 + featuremap
...
class unified(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        ...
        # 动态卷积
        self.conv1 = DynamicConv(args, 64, 4, Add_global_x=True)
        self.conv2 = DynamicConv(args, 128, 4, Add_global_x=True)
        self.conv3 = DynamicConv(args, 256, 4, Add_global_x=True)
        self.conv4 = DynamicConv(args, 512, 4, Add_global_x=True)
        ...
        self.select_global1 = nn.Sequential(
            nn.Linear(1024, 1024 // 4),
            nn.ReLU(inplace=True),
            nn.Linear(1024 // 4, 1024),
            nn.Sigmoid()
        )
        self.select_global2 = nn.Sequential(
            nn.Linear(1024, 1024 // 4),
            nn.ReLU(inplace=True),
            nn.Linear(1024 // 4, 1024),
            nn.Sigmoid()
        )
        self.select_global3 = nn.Sequential(
            nn.Linear(1024, 1024 // 4),
            nn.ReLU(inplace=True),
            nn.Linear(1024 // 4, 1024),
            nn.Sigmoid()
        )
        self.select_global4 = nn.Sequential(
            nn.Linear(1024, 1024 // 4),
            nn.ReLU(inplace=True),
            nn.Linear(1024 // 4, 1024),
            nn.Sigmoid()
        ...
        # self.spacial = nn.Sequential(
        #     nn.Conv2d(1024, 128, 1, 1, 0),
        #     nn.Conv2d(128, 1, 1, 1, 0)
        # )
        # self.spacial_global = nn.Sequential(
        #     nn.Conv2d(1024, 128, 1, 1, 0),
        #     nn.Conv2d(128, 1, 1, 1, 0)
        # )
        # self.spacial_global_x = nn.Sequential(
        #     nn.Conv2d(1024, 128, 1, 1, 0),
        #     nn.Conv2d(128, 1, 1, 1, 0)
        # )
        ...
        self.fc_global_x = nn.Sequential(
            nn.Linear(1024, 128),
            nn.Linear(128, 1),
        )
    ...
    def forward(self, x):
        ...
        featureMap1 = x
        featureMap1 = self.avgpool1(featureMap1)
        featureMap1, featureMap_global1 = self.conv1(featureMap1)
        featureMap1 = self.bn1(featureMap1)
        featureMap1 = self.relu(featureMap1)
        featuremap1 = featureMap1
        importance1 = self.select1(self.avgpool(featureMap1).view(bs, -1)).unsqueeze(2)
        FeatureMap_global1 = self.bn(featureMap_global1)
        FeatureMap_global1 = self.relu(FeatureMap_global1)
        importance_global1 = self.select_global1(self.avgpool(FeatureMap_global1).view(bs, -1)).unsqueeze(2)

        x = self.layer2(x)
        featureMap2 = x
        featureMap2 = self.avgpool1(featureMap2)
        featureMap2, featureMap_global2 = self.conv2(featureMap2)
        featureMap2 = self.bn1(featureMap2)
        featureMap2 = self.relu(featureMap2)
        featuremap2 = featureMap2
        importance2 = self.select2(self.avgpool(featureMap2).view(bs, -1)).unsqueeze(2)
        FeatureMap_global2 = self.bn(featureMap_global2)
        FeatureMap_global2 = self.relu(FeatureMap_global2)
        importance_global2 = self.select_global2(self.avgpool(FeatureMap_global2).view(bs, -1)).unsqueeze(2)

        x = self.layer3(x)
        featureMap3 = x
        featureMap3 = self.avgpool1(featureMap3)
        featureMap3, featureMap_global3 = self.conv3(featureMap3)
        featureMap3 = self.bn1(featureMap3)
        featureMap3 = self.relu(featureMap3)
        featuremap3 = featureMap3
        importance3 = self.select3(self.avgpool(featureMap3).view(bs, -1)).unsqueeze(2)
        FeatureMap_global3 = self.bn(featureMap_global3)
        FeatureMap_global3 = self.relu(FeatureMap_global3)
        importance_global3 = self.select_global3(self.avgpool(FeatureMap_global3).view(bs, -1)).unsqueeze(2)

        x = self.layer4(x)
        featureMap4 = x
        featureMap4 = self.upsample(featureMap4)
        featureMap4, featureMap_global4 = self.conv4(featureMap4)
        featureMap4 = self.bn1(featureMap4)
        featureMap4 = self.relu(featureMap4)
        featuremap4 = featureMap4
        importance4 = self.select4(self.avgpool(featureMap4).view(bs, -1)).unsqueeze(2)
        FeatureMap_global4 = self.bn(featureMap_global4)
        FeatureMap_global4 = self.relu(FeatureMap_global4)
        importance_global4 = self.select_global4(self.avgpool(FeatureMap_global4).view(bs, -1)).unsqueeze(2)
        ...
        featureMap = featureMap1 + featureMap2 + featureMap3 + featureMap4
        # featuremap = torch.cat((featureMap, torch.zeros(featureMap.size(0), featureMap.size(1), featureMap.size(2),
        #                                                 featureMap.size(3)).cuda()), dim=1)

        importance_vector_global = [importance_global1, importance_global2, importance_global3, importance_global4]
        importance_vector_global = torch.cat(importance_vector_global, dim=2)
        importance_vector_global = F.softmax(importance_vector_global, dim=2)
        importance_vector_global = importance_vector_global.unsqueeze(3)

        for i in range(4):
            if i == 0:
                FeatureMap_global1 = FeatureMap_global1 * importance_vector_global[:, :, i, :].unsqueeze(3)
            elif i == 1:
                FeatureMap_global2 = FeatureMap_global2 * importance_vector_global[:, :, i, :].unsqueeze(3)
            elif i == 2:
                FeatureMap_global3 = FeatureMap_global3 * importance_vector_global[:, :, i, :].unsqueeze(3)
            else:
                FeatureMap_global4 = FeatureMap_global4 * importance_vector_global[:, :, i, :].unsqueeze(3)

        FeatureMap_global = FeatureMap_global1 + FeatureMap_global2 + FeatureMap_global3 + FeatureMap_global4

        FeatureMap_global_x = torch.cat((featureMap, self.upsample(x)), dim=1)

        # featuremap = self.avgpool(featuremap).view(x.size(0), -1)
        featuremap_global = self.avgpool(FeatureMap_global).view(x.size(0), -1)
        featuremap_global_x = self.avgpool(FeatureMap_global_x).view(x.size(0), -1)

        # score = self.fc(featuremap)
        score_global = self.fc_global(featuremap_global)
        score_global_x = self.fc_global_x(featuremap_global_x)
        _score = [score_global, score_global_x]
        _score = torch.cat(_score, dim=1)
        _score = F.softmax(_score, dim=1)

        # mask = torch.max(_score, dim=1).values
        # mask = mask.unsqueeze(1)
        # mask = mask.repeat(1, 3)
        # mask = _score == mask
        # _score = mask * _score

        # featuremap = featuremap / torch.norm(featuremap, p=2, dim=1, keepdim=True) * 5
        featuremap_global = featuremap_global / torch.norm(featuremap_global, p=2, dim=1, keepdim=True) * self.alpha
        featuremap_global_x = featuremap_global_x / torch.norm(featuremap_global_x, p=2, dim=1,
                                                               keepdim=True) * self.alpha

        feature = featuremap_global * _score[:, 0].unsqueeze(1) + featuremap_global_x * _score[:, 1].unsqueeze(1)
        # feature = (featuremap + featuremap_global + featuremap_global_x) / 3

        out = self.out(feature)

        return out, featuremap_global_x, featuremap1, featuremap2, featuremap3, featuremap4, feature


class DynamicConv(nn.Module):
    def __init__(self, args, in_feature, ratio=4, only_AConv=False, Add_global=False, Add_global_x=False):
        ...
        self.relu = nn.ReLU()
        self.only_AConv = only_AConv
        self.add_global = Add_global
        self.add_global_x = Add_global_x
    ...
    def forward(self, x):
        ...
        #
        # feature_map = feature_map_1 + feature_map_2 + f  eature_map
        ...
        if self.add_global_x:
            feature_map_global = feature_map1 + feature_map2 + feature_map3 + feature_map4
            feature_map_global = self.upsample(feature_map_global)
            feature_map_global = torch.cat((feature_map_1, feature_map_global), dim=1)
...
class DynamicConv_DHSA(nn.Module):
    def __init__(self, args, in_feature, ratio=4, only_AConv=False, Add_global=False, Add_global_x=False):
        super(DynamicConv_DHSA, self).__init__()
        self.conv1 = nn.Conv2d(in_feature, 512, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        #
        # feature_map = feature_map_1 + feature_map_2 + f  eature_map
        feature_map = feature_map_1 + feature_map1
        # feature_map = torch.cat((feature_map_1, feature_map_global), dim=1)

        return feature_map, 0


class DynamicConv1(nn.Module):
    def __init__(self, args, in_feature, ratio=4, only_AConv=False, Add_global=False, Add_global_x=False):
        super(DynamicConv1, self).__init__()
        self.conv1 = nn.Conv2d(in_feature, 512, 3, 1, 1)
        self.conv2 = nn.Conv2d(in_feature, 512, 3, 1, 1)
        self.conv3 = nn.Conv2d(in_feature, 512, 3, 1, 1)
        self.conv4 = nn.Conv2d(in_feature, 512, 3, 1, 1)
...
class unified_AM_Softmax(nn.Module):
    def __init__(self, args, NumOfLayer, pretrained=True, num_classes=7, drop_rate=0):
        super(unified_AM_Softmax, self).__init__()
        self.drop_rate = drop_rate

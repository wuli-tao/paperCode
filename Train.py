...
parser.add_argument('--Network', type=str, default='AConv+DHSA+global(x)',
                    choices=['Baseline', 'AConv', 'DHSA', 'AConv+DHSA', 'AConv+DHSA+global', 'AConv+DHSA+global(x)',
                             'unified', 'test11', 'test21', 'test_sk', 'test22'])
...
def Test(args, model, test_source_dataloader, test_target_dataloader, Best_Acc_Source, Best_Acc_Target, epoch, writer):
    ...
    if acc_avg > Best_Acc_Source and not args.isTest:
        ...
        #                                                            '{}_{}.pkl'.format(args.sourceDataset,
        #                                                                                   args.Network, )))
        #     else:
        #         torch.save(model.state_dict(), os.path.join(args.OutputPath,
        #                                                     '{}_{}.pkl'.format(args.sourceDataset, args.Network)))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.showFeature:
            VisualizationForTwoDomain(
                '/home/zhongtao/code/CrossDomainFER/my_method/visualization/{}_{}_{:>02d}_test.pdf'.format(

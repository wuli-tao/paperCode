...
parser.add_argument('--sourceDataset', type=str, default='AffectNet',
...
def Train(args, model, ad_net, train_dataloader_source, train_dataloader_target, optimizer, optimizer_ad, scheduler,
          scheduler_ad, epoch, writer):
    """Train."""
    ...
    iter_source_dataloader = iter(train_dataloader_source)
    iter_target_dataloader = iter(train_dataloader_target)

    num_iter = len(train_dataloader_source) if (len(train_dataloader_source) > len(train_dataloader_target)) else len(
        train_dataloader_target)

    for batch_i in trange(num_iter):
        try:
            data_source, label_source, _ = next(iter_source_dataloader)
        except:
            iter_source_dataloader = iter(train_dataloader_source)
            data_source, label_source, _ = next(iter_source_dataloader)

        try:
            data_target, _, _ = next(iter_target_dataloader)
        except:
            iter_target_dataloader = iter(train_dataloader_target)
            data_target, _, _ = next(iter_target_dataloader)
        ...
        imgs = torch.cat((data_source, data_target), 0)
        ...
        # Compute Loss
        global_cls_loss_ = LabelSmoothLoss()(output[:args.batch_size, ], label_source)
        dan_loss_ = DANN(feature, ad_net)
        # mmd_loss_ = MMD_loss()(feature[:args.batch_size, :], feature[args.batch_size:, :])
        ...
        loss_ = global_cls_loss_ + 0.01 * dan_loss_
        ...
        num_ADNet += np.sum(adnet_output[:args.batch_size]) + (args.batch_size - np.sum(adnet_output[args.batch_size:]))
        ...
        Compute_Accuracy(args, output[:args.batch_size, ], label_source, acc, prec, recall)
    ...
    LoggerInfo += '''    AdversarialNet Acc {0:.4f} Acc_avg {1:.4f} Prec_avg {2:.4f} Recall_avg {3:.4f} F1_avg {4:.4f} MMD Loss {5:.4f} Total Loss {loss:.4f}\n'''.format(
        num_ADNet / (2.0 * args.batch_size * batch_i), acc_avg, prec_avg, recall_avg, f1_avg, 0,
        loss=loss.avg)
        ...
        if not args.isTest and args.isSave:
            ...
            if isinstance(model, nn.DataParallel):
                torch.save(model.module.state_dict(), os.path.join(args.OutputPath,
                                                                   '{}_{}_Adaption_{}.pkl'.format(args.sourceDataset,
                                                                                          args.targetDataset,
                                                                                          args.Network, )))
            else:
...
def main():
    ...
    if not args.isTest:
        sys.stdout = Logger(
            osp.join('./Logs/', '{}_{}_Adaption_{}.txt'.format(args.sourceDataset, args.targetDataset, args.Network)))
    if args.seed:
    ...
    if args.sourceDataset == 'RAFDB':
        train_dataset_source = RafDataSet(args.raf_path, phase='train', transform=data_transforms, basic_aug=True)
    elif args.sourceDataset == 'FER2013':
        train_dataset_source = FER2013DataSet(args.fer2013_path, phase='train', transform=data_transforms,
                                              basic_aug=True)
    elif args.sourceDataset == 'FER2013Plus':
        train_dataset_source = FER2013PlusDataSet(args.fer2013plus_path, phase='train', transform=data_transforms,
                                                  basic_aug=True)
    elif args.sourceDataset == 'SFEW':
        train_dataset_source = SFEWDataSet(args.sfew_path, phase='train', transform=data_transforms, basic_aug=True)
    elif args.sourceDataset == 'ExpW':
        train_dataset_source = ExpWDataSet(args.expw_path, phase='train', transform=data_transforms, basic_aug=True)
    elif args.sourceDataset == 'AffectNet':
        train_dataset_source = AffectNetDataSet(args.affectnet_path, phase='train', transform=data_transforms,
                                                basic_aug=True)

    if args.targetDataset == 'RAFDB':
        train_dataset_target = RafDataSet(args.raf_path, phase='train', transform=data_transforms, basic_aug=True)
    elif args.targetDataset == 'JAFFE':
        train_dataset_target = JAFFEDataSet(args.jaffe_path, phase='train', transform=data_transforms, basic_aug=True)
    elif args.targetDataset == 'FER2013':
        train_dataset_target = FER2013DataSet(args.fer2013_path, phase='train', transform=data_transforms,
                                              basic_aug=True)
    elif args.targetDataset == 'FER2013Plus':
        train_dataset_target = FER2013PlusDataSet(args.fer2013plus_path, phase='train', transform=data_transforms,
                                                  basic_aug=True)
    elif args.targetDataset == 'SFEW':
        train_dataset_target = SFEWDataSet(args.sfew_path, phase='train', transform=data_transforms, basic_aug=True)
    elif args.targetDataset == 'ExpW':
        train_dataset_target = ExpWDataSet(args.expw_path, phase='train', transform=data_transforms, basic_aug=True)
    elif args.targetDataset == 'AffectNet':
        train_dataset_target = AffectNetDataSet(args.affectnet_path, phase='train', transform=data_transforms,
                                                basic_aug=True)

    print('Source Train set size:', train_dataset_source.__len__())
    print('The Source Train dataset distribute:', train_dataset_source.__distribute__())
    train_loader_source = torch.utils.data.DataLoader(train_dataset_source,
                                                      batch_size=args.batch_size,
                                                      num_workers=args.workers,
                                                      shuffle=True,
                                                      pin_memory=True,
                                                      drop_last=True)
    print('Target Train set size:', train_dataset_target.__len__())
    print('The Target Train dataset distribute:', train_dataset_target.__distribute__())
    train_loader_target = torch.utils.data.DataLoader(train_dataset_target,
                                                      batch_size=args.batch_size,
                                                      num_workers=args.workers,
                                                      shuffle=True,
                                                      pin_memory=True,
    ...
    if args.targetDataset == 'RAFDB':
        ...
        val_dataset_target = JAFFEDataSet(args.jaffe_path, phase='test', transform=data_transforms_val)
    elif args.targetDataset == 'FER2013':
    ...
    ad_net = BulidAdversarialNetwork(args, model.output_num(), args.class_num)
    print('Done!')
    ...
    scheduler_ad = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_ad, T_max=args.epochs, eta_min=1e-6)
    print('Done!')
    ...
    writer = SummaryWriter(os.path.join('/home/zhongtao/code/CrossDomainFER/my_method/LogInfo',
                                        '{}_{}_Adaption_{}'.format(args.sourceDataset, args.targetDataset,
                                                                   args.Network)))

    for epoch in range(1, args.epochs + 1):

        if not args.isTest:
            Train(args, model, ad_net, train_loader_source, train_loader_target, optimizer, optimizer_ad, scheduler,
                  scheduler_ad,
                  epoch, writer)
